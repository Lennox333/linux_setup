...
const CloseBtn = ({ func }: { func: () => void }) => {
  return (
    <button className="close-button" onClick={func} css="all:unset;">
      <icon icon={icons.ui.close} css="font-size: 20px;" />
    </button>
  );
};
...
const Actions = ({
  ...
  const buttons = actions.map(({ label, id }) => (
    <button
...
export default function Notification(props: NotificationProps) {
  // const { noti, setup, onClick, width = 40 } = props;
  const { noti, setup, width = 40 } = props;
  const { START, CENTER, END } = Gtk.Align;
  return (
    <eventbox setup={setup}>
      <box
      ...
      >
        ...
        <Content noti={noti} />
        {/* <Actions
          actions={noti.get_actions()}
          invoke={noti.invoke}
          dismiss={noti.dismiss}
        /> */}
        <box className="actions-container" spacing={1}>
          {noti.actions.map(({ label, id }) => (
            <button
              hexpand
              onClick={() => {
                noti.invoke(id);
